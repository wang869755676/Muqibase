...
public class CrashHandler implements UncaughtExceptionHandler {
	...
	private boolean handleException(Throwable ex) {
		...
		if (ex.getStackTrace() == null && ex.getCause() == null
				&& ex.getLocalizedMessage() == null) {
			msg1 = "程序出错:[" + ex + " .]";
		}
		final String msg = msg1;
		// final String msg =
		// "程序:["+ex.getCause().getStackTrace()[0]+" .]"+"\n"+"出现错误:["+ex.getLocalizedMessage()+" .]"+"\n"+"错误原因:["+ex.getCause()+" .]";
		// final String msg =
		// "程序:["+ex.getCause().getStackTrace()[0]+ex.getCause().getStackTrace()[1]+ex.getCause().getStackTrace()[2]+ex.getCause().getStackTrace()[3]+ex.getCause().getStackTrace()[4]+ex.getCause().getStackTrace()[5]+ex.getCause().getStackTrace()[6]+" .]"+ex.getCause();
		// final String msg =
		// "[程序:  "+((ex.getCause().getStackTrace()[0]).toString()).split("\\(")[1]+" 出现错误: "+ex.getCause()+".] ";
		// 使用Toast来显示异常信息
		new Thread() {
			@Override
			public void run() {
				...
				Looper.prepare();
				// ShowToast.showShort(context, msg);
				Log.d(TAG, msg);
				Looper.loop();
			}
		}.start();
